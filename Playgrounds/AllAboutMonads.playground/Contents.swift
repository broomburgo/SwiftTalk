...
/*:
- a (twice generic) operator `>>-`, usually called `bind`, that builds a new instance of `Maybe` from an existing instance and a function that creates the new one from the value *contained* in the first one.
*/
...
To give and example of verification of the monad laws, we'll define a vary simple monad called `Box<A>` that simply "boxes" a value.
*/
class Box<A: Equatable>: Equatable {
	let value: A
	init(_ value: A) {
		self.value = value
	}

	static func unit(x: A) -> Box {
		return Box(x)
	}
}

func >>- <A,B> (lhs: Box<A>, rhs: A -> Box<B>) -> Box<B> {
	return rhs(lhs.value)
}

func == <A>(lhs: Box<A>, rhs: Box<A>) -> Bool {
	return lhs.value == rhs.value
}
/*:
To state that `Box<A>` is **actually** a monad, we need to prove for it the following laws:
*/
extension Box {
	static func firstLaw(f f: A -> Box<A>) -> A -> Bool {
		return { x in (Box.unit(x) >>- f) == f(x) }
	}

	static func secondLaw() -> A -> Bool {
		return { x in Box.unit(x) >>- Box.unit == Box.unit(x) }
	}

	static func thirdLaw(f f: A -> Box<A>, g: A -> Box<A>) -> A -> Bool {
		return { x in Box.unit(x) >>- f >>- g == Box.unit(x) >>- { a in f(a) >>- g } }
	}
}
/*:
To check the laws validity for `Box<A>`, we'll simply apply the laws to an array of random integers, and check if they stand for every integer: the proofs are not completely general becuase we'll pass into the laws some specific `f` and `g` functions, which means that we are not really testing the laws against "every" function, but the proofs will be sufficient for the present case.
*/
...
checkLaw(Box<Int>.firstLaw(f: { Box($0*$0) }))
checkLaw(Box<Int>.secondLaw())
checkLaw(Box<Int>.thirdLaw(f: { Box($0*$0) }, g: { Box($0*2) }))
/*:
#### One-way monads

Notice that in our simple monad called `Box<A>` we have a way to retrieve the contained value of type `A`: that's not a requirement for monads, and plenty of monads don't allow it; these monads can be considered *one-way*: once you put the value in, there's no way to retrieve it directly.
